...
function RouteComponent() {
  ...
  useEffect(() => {
    if (roadmaps.length > 0) {
      ...
      router.navigate({ to: '/admin/roadmap/$roadmapSlug', params: { roadmapSlug: roadmaps[0].slug } });
    }
    ...
    <div className='pt-18 w-full h-full vertical'>
      <div className='horizontal center-v gap-2 p-4 border-b'>
        <SelectComponent
          ...
          onChange={(value) => {
            if (value)
              router.navigate({ to: '/admin/roadmap/$roadmapSlug', params: { roadmapSlug: value } });
          }}
        ...
        <RoadmapMenu
          ...
          onRename={() => {
            if (roadmapSlug)
              router.navigate({ to: '/admin/roadmap/$roadmapSlug/rename', params: { roadmapSlug } });
          }}
          onDuplicate={() => {
            if (roadmapSlug)
              duplicateRoadmap();
          ...
          onDelete={roadmaps.length > 1 ? () => {
            if (roadmapSlug)
              router.navigate({ to: '/admin/roadmap/$roadmapSlug/delete', params: { roadmapSlug } });
          } : undefined}
        />
        ...
        />
        {roadmapSlug && <Link to="/admin/roadmap/$roadmapSlug/filter" params={{ roadmapSlug }} className='ml-auto'>
          <Button role='button' id='filter-button' className='h-9 px-4 gap-4' variant='outline' size='sm' color='secondary'>
            <Icons.Filter />
            Filters
          </Button>
        </Link>}
        ...
        <div className='h-5 border-l' />
        {roadmapSlug && <Link to="/admin/roadmap/$roadmapSlug/new" params={{ roadmapSlug }}>
