...
AND feedback.status IN ('PLANNED', 'IN_PROGRESS')
GROUP BY feedback.id, feedback.title, feedback.status, feedback.slug, board.slug, feedback."createdAt", CASE WHEN votes.id IS NOT NULL THEN TRUE ELSE FALSE END
